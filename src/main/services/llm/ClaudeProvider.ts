...
import { SCHEMA_VERSION, DEFAULT_CLAUDE_MODEL } from '@shared/types'
...
export class ClaudeProvider implements LLMProvider {
  ...
  async analyzeCommit(input: AnalyzeCommitInput): Promise<AnalysisResult> {
    ...
    // Attempt 1
    let attempt = await callClaude(client, system, userMessage, modelId, input.signal)
    ...
    try {
      ...
      try {
        attempt = await callClaude(client, system, retryMessage, modelId, input.signal)
        totalIn += attempt.usage.input
        totalOut += attempt.usage.output
        parsedRaw = JSON.parse(stripJsonFence(attempt.text))
      } catch {
